...
parser.add_argument('--rgepoch', default=20, type=int,
                    help='regeneration epoch')
...
#         print('Layer id: {} kernel_size: {}'.format(k,m.kernel_size))
#     if isinstance(m,nn.BatchNorm2d):
#         print('Layer id: {}'.format(k))
...
def save_checkpoint(state, is_best, epoch, filepath):
    if is_best:
        ...
        filename = os.path.join(filepath, 'best_model.pth.tar')
...
def bn_analysis(epoch):
    layer_count = 1
    for k1,m1 in enumerate(model.modules()):
        if isinstance(m1, nn.Conv2d) and (m1.kernel_size == (3,3)):
            #print("conv found in : {}".format(k1))
            for k,m in enumerate(model.modules()):
                if isinstance(m, nn.BatchNorm2d) and (k == k1+1):
                    #print("bn for conv3x3 found in : {}".format(k))
                    size = m.weight.data.shape[0]
                    bn_score = np.zeros(size)
                    bn_layer_values = m.weight.data.abs().clone()
                    bn_score[0:size] = bn_layer_values.cpu().numpy()
                    str_file = str('rj_layer' + str(layer_count) + '.txt')
                    np.savetxt(str_file, bn_score, fmt = '%10.5f', delimiter=',')
                    layer_count += 1
                    break
...
def rejuvenate():
    ...
    #np.savetxt('layer1.txt', bn_score, fmt = '%10.5f', delimiter=',')
    for k,m in enumerate(model.modules()):
        if isinstance(m,nn.BatchNorm2d):
            # reset_tensor = torch.ones_like(m.weight.data) * args.rgval
            # m.weight.data = torch.where(m.weight.data.abs() < mThre.cuda(),reset_tensor,m.weight.data)
...
for epoch in range(args.start_epoch, args.epochs):
    ...
    if is_best:
        ...
        }, is_best, epoch+1, filepath=args.save)

    #bn_analysis(epoch)
    if (epoch%args.rgepoch) == 0:
        rejuvenate()
